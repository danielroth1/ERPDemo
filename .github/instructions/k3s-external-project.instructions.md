...
under a subdomain of `shopping-now.net` (e.g. `my-project.shopping-now.net`).
The user tells you which subdomain to use. Each project runs in its own
Kubernetes namespace with automatic TLS via Let's Encrypt.
...
1. **Copy** this file into the other project:
   ...
   ```
2. **Tell the AI** what you want. You provide the subdomain — the AI reads
   this file and generates deployment artifacts. You do **not** edit
   placeholders — the AI fills them in. Example prompts:
   > "Set up k3s deployment for my-project.shopping-now.net"
   > "Generate k3s deploy scripts for this project as my-other-project.shopping-now.net"
...
3. **What the AI creates** (and then you run yourself):
   ...
   - `HOW_TO_DEPLOY.md` — step-by-step deployment instructions for developers
   - `.env.example` — environment variable template (if the project doesn't already have one)
...
>
> **Do I need to copy `.env.deploy`?** Create a minimal `.env.deploy` with these 4 lines
> (replace the values with your own):
> ```bash
> K3S_SERVER="<username>@<ip>"
> K3S_SSH_PORT="7878"
> YOURUSER="your-github-username"
...
Browser ──► k3s Server (port 80/443)
              └── Traefik (built-in, kube-system)
                    ...
                    ▼
                  Ingress: host=SUBDOMAIN.shopping-now.net
                    └──► SERVICE_NAME-service (ClusterIP, namespace: PROJECT_NAME)
                           └──► APP_NAME Pod  (your app container)
```
...
When asked to "set up k3s deployment" for a project, **first collect the
required information from the user** if they haven't already provided it.
Ask the user for:

1. **Subdomain** — the part before `.shopping-now.net` (e.g. if they want
   `my-app.shopping-now.net`, the subdomain is `my-app`)
2. **GitHub username** — used for the container registry (`ghcr.io/USER/...`)
3. **Database needs** — does the project need its own PostgreSQL, Redis,
   MongoDB, or other containerized service? If yes, generate an additional
   `k8s/postgres.yaml` (or equivalent) manifest using the patterns in the
   "Deploying Your Own Database Containers" section below.

If the user gives the full domain (e.g. `my-app.shopping-now.net`), extract
`SUBDOMAIN` as the part before `.shopping-now.net`.

Once you have both values, generate these files.
Derive `PROJECT_NAME` from the subdomain or project folder name (lowercase,
hyphens allowed). Detect the Dockerfile and the port the app listens on by
reading the project.
...
Generate a `HOW_TO_DEPLOY.md` file at the project root with step-by-step
instructions for a developer who has just cloned the repo. Use `PROJECT_NAME`
and `SUBDOMAIN` as detected from the user's prompt.

```markdown
# Deploy PROJECT_NAME

This project runs on the k3s cluster at `SUBDOMAIN.shopping-now.net`.
...
- Logged into `ghcr.io` — you need a GitHub PAT with `read:packages` and `write:packages` scopes:
...
|----------|-------------|
| `K3S_SERVER` | SSH target for the k3s server (e.g. `<user>@<ip>`) |
| `K3S_SSH_PORT` | SSH port (default: `7878`) |
| `YOURUSER` | Your GitHub username |
| `GHCR_TOKEN` | A GitHub PAT with `read:packages` + `write:packages` scopes |
...
Or, if you have VS Code, open the project and run the **k3s: deploy** task
from the Terminal → Run Task… menu.

### One-command build, push & deploy

```bash
bash scripts/deploy.sh   # builds, pushes, and deploys all at once
```

## Checking status

```bash
KUBECONFIG=~/.kube/k3s-erp.yaml kubectl get pods,ingress,svc,certificate -n PROJECT_NAME -o wide
```

The app will be available at `https://SUBDOMAIN.shopping-now.net`.
TLS certificate provisioning may take 2–5 minutes on the first deploy.

## Updating

After making changes, just run `bash scripts/deploy.sh` again — it builds a
new image, pushes it, and applies the manifests.
```

> Place the generated file at the project root (`HOW_TO_DEPLOY.md`).
> The `.env.example` file should contain the same variables as described
> in the Prerequisites section of this instruction file. Create it if it
...
| `SUBDOMAIN` | From the user's prompt — the part before `.shopping-now.net` (e.g. "my-project" from "my-project.shopping-now.net") |
| `YOURUSER` | From the project's `.env.deploy` → `YOURUSER=...` or ask the user |
...
## Deploying Your Own Database Containers

External projects can deploy their own database containers (PostgreSQL, Redis,
MongoDB, etc.) inside their namespace alongside the app. This is the
**recommended approach** — each project gets full isolation with its own
credentials and lifecycle.

### Dedicated database in your namespace

Add a `k8s/postgres.yaml` manifest. Example for PostgreSQL:
...
stringData:
  ...
  connection-string: "Host=PROJECT_NAME-postgres;Port=5432;Database=myapp;Username=myapp;Password=changeme123"
---
apiVersion: v1
...
Then reference it from your app container via the service name (same-namespace
DNS, no cross-namespace qualifier needed):
...
If you want your app to wait for the database to be ready before starting,
...
initContainers:
  - name: wait-for-postgres
    image: busybox:1.36
    command: ['sh', '-c', 'until nc -z PROJECT_NAME-postgres 5432; do echo "Waiting for postgres..."; sleep 3; done']
```

> `kubectl apply -f k8s/` applies all YAML files in the directory, so the
> deploy script picks up the database manifest automatically — no changes
> to `deploy.sh` needed.

### Resource overhead

| Database | Image | Typical RAM | Disk |
|----------|-------|-------------|------|
| PostgreSQL | `postgres:16-alpine` | 128–512 Mi | 1 Gi |
| Redis | `redis:7-alpine` | 64–256 Mi | — |
| MongoDB | `mongo:7` | 256–1024 Mi | 1 Gi |
| Kafka | `confluentinc/cp-kafka:7.5.0` | 1–4 Gi | 2 Gi |

For lightweight databases, the overhead is modest — you can run several
per-project databases on a single k3s node.
...
# If stale, re-run the setup script from the ERPDemo repo or re-export the kubeconfig
```
